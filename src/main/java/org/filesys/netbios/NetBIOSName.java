...
public class NetBIOSName {
     ...
     */
    public NetBIOSName(String name, char typ, boolean group, List<byte[]> ipList) {
        setName(name);
        setType(typ);
        setGroup(group);
     ...
     * @param ttl    int
     */
    public NetBIOSName(String name, char typ, boolean group, List<byte[]> ipList, int ttl) {
        setName(name);
        setType(typ);
        setGroup(group);
        addIPAddresses(ipList);
     ...
     */
    public final void addIPAddresses(List<byte[]> addrList) {
        if (m_addrList == null)
            m_addrList = new ArrayList<byte[]>();
